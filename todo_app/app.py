...
from todo_app.data.trello_items import get_items, get_lists, add_item, get_list_id, change_status
...
def move_to_done(item_id):
    list_id = get_list_id(list_names, 'Done')
    change_status(item_id, trello_token, trello_key, list_id)
    return redirect(url_for('index'))


@app.route('/<item_id>/in_progress', methods=['POST', 'PUT', 'GET'])
def move_to_in_progress(item_id):
    list_id = get_list_id(list_names, 'In Progress')
    change_status(item_id, trello_token, trello_key, list_id)
    return redirect(url_for('index'))
...
def move_to_to_do(item_id):
    list_id = get_list_id(list_names, 'To Do')
    change_status(item_id, trello_token, trello_key, list_id)
    return redirect(url_for('index'))
